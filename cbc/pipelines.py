...
class CbcPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `cbc`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM cbc WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO cbc (date, title, link, content)"
